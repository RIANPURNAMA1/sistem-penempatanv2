...
const getAll = async (req, res) => {
  try {
    const { search, status, cabang_id } = req.query;
    const user = req.user;
    let query = `
      SELECT kp.*, u.nama, u.email, u.status as user_status, c.nama_cabang
...
const uploadDokumen = async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [profil[0].id, jenis_dokumen, req.file.originalname, req.file.path, req.file.size, req.file.mimetype]
    );
    res.json({ success: true, message: 'Dokumen berhasil diupload', path: req.file.path });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, message: 'Server error' });
  }
