...
const migrations = [
  ...
  {
    name: 'add_penghasilan_anggota_keluarga',
    check: async (conn) => {
      const [cols] = await conn.query('SHOW COLUMNS FROM kandidat_keluarga LIKE "penghasilan"');
      return cols.length > 0;
    },
    sql: `ALTER TABLE kandidat_keluarga ADD COLUMN penghasilan VARCHAR(50) AFTER pekerjaan`
  },
  {
    name: 'add_suami_istri_keluarga',
    check: async (conn) => {
      const [cols] = await conn.query("SHOW COLUMNS FROM kandidat_keluarga LIKE 'hubungan'");
      if (cols.length === 0) return false;
      const enumValues = cols[0].Type.replace('enum(', '').replace(')', '').replace(/'/g, '').split(',');
      return enumValues.includes('Suami') && enumValues.includes('Istri');
    },
    sql: `ALTER TABLE kandidat_keluarga MODIFY COLUMN hubungan ENUM('Ayah', 'Ibu', 'Suami', 'Istri', 'Kakak', 'Adik', 'Lainnya') NOT NULL`
  },
  {
