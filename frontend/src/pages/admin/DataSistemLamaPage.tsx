...
export default function DataSistemLamaPage() {
  const [activeTab, setActiveTab] = useState<
    "dashboard" | "pendaftaran" | "kandidat" | "terverifikasi" | "datacv"
  >("dashboard");
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/kandidat`)
      .then((r) => r.json())
      .then((r) => {
        if (r.success) setKandidatData(r.data);
      })
      .catch(() => {});
  }, []);
  useEffect(() => {
  ...
  const sswStats = Object.keys(sswStatsMap)
    .map((ssw) => ({
      ssw,
  ...
  kandidatData.forEach((k) => {
    const p = k.status_kandidat || "Unknown";
    progressStats[p] = (progressStats[p] || 0) + 1;
  });
    ...
    <div className="page-container p-6 overflow-visible">
      ...
      <div className="flex gap-2 mb-6 flex-wrap overflow-x-auto">
        {tabs.map((tab) => (
          ...
          >
            <tab.icon size={14} className="mr-1 md:mr-2" />{" "}
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <span
                        className={`px-2 py-1 rounded text-xs ${item.verifikasi === "diterima" ? "bg-green-100 text-green-700" : item.verifikasi === "ditolak" ? "bg-red-100 text-red-700" : "bg-yellow-100 text-yellow-700"}`}
                      >
                        {item.verifikasi || "-"}
                      </span>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      ...
                      <div>
                        ...
                        <p className="text-green-600">{item.no_wa || "-"}</p>
                      </div>
                    </div>
                    <Button
                      variant="outline"
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="px-3 py-2 text-center">VERIFIKASI</th>
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2 text-xs">
                          <div>{item.email}</div>
                          <div className="text-green-600">{item.no_wa}</div>
                        </td>
                        <td className="px-3 py-2 text-center">
                          <span
                            className={`px-2 py-1 rounded text-xs ${item.verifikasi === "diterima" ? "bg-green-100 text-green-700" : item.verifikasi === "ditolak" ? "bg-red-100 text-red-700" : "bg-yellow-100 text-yellow-700"}`}
      ...
      {activeTab === "kandidat" && (
        <KandidatTable onSelect={handleSelectKandidat} />
      )}
      ...
      {activeTab === "terverifikasi" && (
        <div className="overflow-visible">
          ...
          <div className="md:hidden space-y-3 px-4 pb-4">
            {loading ? (
            ...
            ) : filteredData.filter((d) => d.verifikasi === "diterima")
                .length === 0 ? (
              <div className="py-12 text-center text-gray-500">
                Tidak ada data
              </div>
              ...
              filteredData
                .filter((d) => d.verifikasi === "diterima")
                .map((item, idx) => (
                  <div
                    key={item.id}
                    className="bg-white rounded-lg border shadow-sm p-4 space-y-3"
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      ...
                      <div>
                        <span className="text-gray-400">Status JFT</span>
                        <span
                          className={`ml-1 px-2 py-0.5 rounded-full text-xs ${item.status_jft === "sudah ujian jft" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                        >
                          {item.status_jft || "-"}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-400">Status SSW</span>
                        <span
                          className={`ml-1 px-2 py-0.5 rounded-full text-xs ${item.status_ssw === "sudah ujian ssw" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                        >
                          {item.status_ssw || "-"}
                        </span>
                      </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left">Status JFT</th>
                  <th className="px-3 py-2 text-left">Status SSW</th>
                </tr>
              </thead>
              <tbody className="divide-y">
                {loading ? (
                    ...
                    .map((item, idx) => (
                      <tr key={item.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2">
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs ${item.status_jft === "sudah ujian jft" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                          >
                            {item.status_jft || "-"}
                          </span>
                        </td>
                        <td className="px-3 py-2">
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs ${item.status_ssw === "sudah ujian ssw" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                          >
                            {item.status_ssw || "-"}
                          </span>
                        </td>
          ...
          <DialogContent className="max-w-4xl">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => generatePendaftaranExcel(selectedData)}
                  >
                    <Download size={14} className="mr-2" /> Download Excel
                  </Button>
                  <Button
                    size="sm"
                    onClick={() => generatePendaftaranPDF(selectedData)}
                  >
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <DetailItem label="Agama" value={selectedData.agama} />
                <DetailItem
                  label="Pendidikan"
                  value={selectedData.pendidikan_terakhir}
                />
              </div>
              <div>
                <h4 className="font-bold text-blue-600 text-sm mb-2">
                ...
                <DetailItem
                  label="ID Prometric"
                  value={selectedData.id_prometric}
                />
                <DetailItem
                  label="Status JFT"
                  value={selectedData.status_jft}
                />
                <DetailItem
                  label="Status SSW"
                  value={selectedData.status_ssw}
                />
              </div>
              <div>
                <h4 className="font-bold text-blue-600 text-sm mb-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <LinkFileWithDownload
                    label="Pas Foto"
                    path={selectedData.dokumen?.foto}
                  />
                  <LinkFileWithDownload
                    label="KTP"
                    path={selectedData.dokumen?.ktp}
                  />
                  <LinkFileWithDownload
                    label="KK"
                    path={selectedData.dokumen?.kk}
                  />
                  <LinkFileWithDownload
                    label="Ijazah"
                    path={selectedData.dokumen?.ijasah}
                  />
                  <LinkFileWithDownload
                    label="Akte Kelahiran"
                    path={selectedData.dokumen?.akte}
                  />
                  <LinkFileWithDownload
                    label="Bukti Pelunasan"
                    path={selectedData.dokumen?.bukti_pelunasan}
                  />
                  <LinkFileWithDownload
                    label="Sertifikat JFT"
                    path={selectedData.dokumen?.sertifikat_jft}
                  />
                  <LinkFileWithDownload
                    label="Sertifikat SSW"
                    path={selectedData.dokumen?.sertifikat_ssw}
                  />
                </div>
              </div>
            </div>
      ...
      {showKandidatModal && selectedKandidat && (
        <Dialog open={showKandidatModal} onOpenChange={setShowKandidatModal}>
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                Detail Kandidat: {selectedKandidat?.pendaftaran?.nama}
              </DialogTitle>
            </DialogHeader>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <DetailItem
                  label="NIK"
                  value={selectedKandidat.pendaftaran?.nik}
                />
                <DetailItem
                  label="No WA"
                  value={selectedKandidat.pendaftaran?.no_wa}
                />
                <DetailItem
                  label="Email"
                  value={selectedKandidat.pendaftaran?.email}
                />
              </div>
              <div>
                ...
                <DetailItem
                  ...
                  value={selectedKandidat.status_kandidat}
                />
                <DetailItem
                  label="Perusahaan"
                  value={selectedKandidat.nama_perusahaan}
                />
                <DetailItem
                  label="Cabang"
                  value={selectedKandidat.cabang?.nama_cabang}
                />
              </div>
            </div>
          </DialogContent>
      ...
      {pathArray.map((p, i) => {
        ...
        return (
          <a
            ...
            className="block p-2 border border-blue-200 rounded text-blue-600 text-xs text-center hover:bg-blue-50"
          >
