            ...
            paginatedData.map((item, idx) => (
              <div key={item.id} className="bg-white rounded-lg border shadow-sm p-4 space-y-3">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="font-medium text-gray-900 text-sm">{item.nama_perusahaan}</p>
                    <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium mt-1 ${item.status === 'aktif' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                      {item.status === 'aktif' ? 'Aktif' : 'Nonaktif'}
                    </span>
                  </div>
                  <span className="text-xs text-gray-400">#{((page - 1) * perPage) + idx + 1}</span>
                </div>
                
                <div className="grid grid-cols-1 gap-2 text-xs">
                  <div>
                    <span className="text-gray-400">Alamat</span>
                    <p className="text-gray-600">{item.alamat_indonesia || item.alamat || '-'}</p>
                  </div>
                  <div>
                    <span className="text-gray-400">Kontak</span>
                    <p className="text-gray-600">{item.kontak_pic || item.kontak || '-'}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-blue-600">{item.website || '-'}</p>
                  </div>
                  {item.keterangan && (
                    <div>
                      <span className="text-gray-400">Keterangan</span>
                      <p className="text-gray-500">{item.keterangan}</p>
                    </div>
                  )}
                </div>
